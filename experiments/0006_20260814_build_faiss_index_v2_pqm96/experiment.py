...
def main() -> None:
    ...
    logger.info(f"dim={dim} nlist={nlist} pq_m={pq_m} pq_nbits={pq_nbits} seed={seed}")
    ...
    for p in (manifest_path, slide_meta_path, training_sample_path):
        if not p.exists():
            raise FileNotFoundError(
                f"{p} not found — run experiments/0004_..._build_patch_manifest_v2 first"
            )
    ...
    # this one run_dir instead of also reaching back into 0004's outputs.
