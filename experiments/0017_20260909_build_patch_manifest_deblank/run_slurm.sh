#!/bin/bash
#SBATCH --job-name=0017_20260909_build_patch_manifest_deblank
#SBATCH --partition=medium-creator-i
#SBATCH --output=/workspace/filesrv02/kito/patch-vector-search/logs/0017_20260909_build_patch_manifest_deblank/%j_0017_20260909_build_patch_manifest_deblank.out
#SBATCH --error=/workspace/filesrv02/kito/patch-vector-search/logs/0017_20260909_build_patch_manifest_deblank/%j_0017_20260909_build_patch_manifest_deblank.out
...
#SBATCH --mem=48g
#SBATCH --time=2:00:00
# CPU-only: coords走査+1000ファイル中~100枚だけfeaturesフル読み込み(学習サンプル収集)、
# それに加えて corpus_blankness.parquet(~293MB、18.4M行)を読み sat_frac < 0.10 の
# 背景行を除外する軽量パス。GPU不要なので--gres=gpu:1は外している。
# 0010(macenko manifest、8cpu/32g/2h)を踏襲。blankness parquet読み込み分の
# 余裕を見て mem を 48g に上げている。
...
# #SBATCH --output=/workspace/filesrv02/kito/patch-vector-search/logs/0017_20260909_build_patch_manifest_deblank/%A_%a_0017_20260909_build_patch_manifest_deblank.out
# #SBATCH --error=/workspace/filesrv02/kito/patch-vector-search/logs/0017_20260909_build_patch_manifest_deblank/%A_%a_0017_20260909_build_patch_manifest_deblank.out
# #SBATCH --array=0-N
#
...
export EXP_NAME="0017_20260909_build_patch_manifest_deblank"
