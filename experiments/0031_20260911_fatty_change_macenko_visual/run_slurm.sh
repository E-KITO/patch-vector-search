...
# job 10599 で max_tiles_reranked=12 だとギャラリーが空になった(8図版・501
# タイルに対して再ランク対象が少なすぎ、n_hits_ratioベースのtop_slidesと
# 厳密re-rankのhit_poolがほぼ重ならなかった)ため、全タイル厳密re-rankに変更
# (config.yml の max_tiles_reranked: null)— NFS への h5 ランダムアクセスが
# 増えるため time を 4h に拡大。生h5/生WSIともNFS直読み(局所SSDステージング
# は行わない、対象が1所見×2索引のみと小さいため)。
#
