...
SUPPORTED_FINDINGS = (
    ...
    # 2026-09-18 追加: self_retrieval_diagnostic
    # (outputs/gt_validations/self_retrieval_diagnostic.csv) で「バッチ交絡なし
    # (batch_dominates_finding_rate=0.0)・nhr_best_rank_med_noexp が一桁台」と
    # 確認済みの3所見。コーパス全体のFINDING_TYPEラベル数(single_finding_liver.csv
    # 単体では339/130/142枚)だけを見ると豊富に見えるが、この索引が実際に埋め込んで
    # いるコーパスに限るとGT枚数はそれぞれ16/7/7枚しかなく、glycogen(6枚)と
    # 同水準のスパースさ——単純にGT付きスライドから切り出すだけでは「代表パッチ集」
    # と呼べる量・多様性に届かない、検索での拡張が意味を持つ所見という点でglycogen/
    # ground glassと同じ立場にある。finding_routing.py のMACENKO_FINDINGS/
    # WHITEN_FINDINGSどちらにも含まれないため baseline(0018)固定。
    "Microgranuloma",
