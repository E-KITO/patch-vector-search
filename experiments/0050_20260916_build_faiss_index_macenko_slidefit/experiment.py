"""Build the OPQ+IVF+PQ FAISS index for the slide-unit-fit uni_v1 Macenko corpus.

wsi_preprocess側が experiments/0043・0044 の事前検証を受けて再構築した
「スライド単位fit」Macenkoコーパス(features_uni_v1_macenko_slidefit、README
「wsi_preprocess連携」参照)を、experiments/0049(背景除去済みmanifest)を
入力に、既存Macenko索引(experiments/0012→0033、パッチ単位fit)と全く同じ
ハイパラ(nlist=4096, pq_m=64, pq_nbits=8, opq_niter=10, seed=42)で構築した
もの。experiment.py 本体は experiments/0033 のコピー(ロジック変更なし、
config.yml の manifest_exp_dir/features_dir が0049・スライド単位fitコーパス
を指す点だけが違う)。

Downstream: experiments/0051(この索引と既存パッチ単位fit索引をGT best_rank
で比較する最終判定)。比較結果次第で lib.finding_routing.MACENKO_INDEX_DIR /
MACENKO_FEATURES_DIR の差し替えを検討する。
"""
import argparse
import json
import logging
...
def main() -> None:
    ...
    for p in (manifest_path, slide_meta_path, training_sample_path):
        if not p.exists():
            raise FileNotFoundError(
                f"{p} not found — run experiments/0049_..._build_patch_manifest_macenko_slidefit first"
            )
    ...
    # (experiments/0051's GT comparison) only need to point at this one
    # run_dir instead of also reaching back into 0049's outputs.
