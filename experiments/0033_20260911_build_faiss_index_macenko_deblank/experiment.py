"""Build the OPQ+IVF+PQ FAISS index for the background-removed uni_v1 Macenko corpus.

experiments/0012 の Macenko 索引は背景除去前(0002相当のハイパラ)で構築されて
おり、現行 baseline(0018、背景除去済み)と同一コーパス世代ではなかった
(README「experiments/0031」参照)。これは experiments/0032(背景除去適用済みの
Macenko manifest)を入力に、0012/0018 と全く同じハイパラ(nlist=4096, pq_m=64,
pq_nbits=8, opq_niter=10, seed=42)で索引を再構築したもの。experiment.py 本体は
experiments/0012 のコピー(ロジック変更なし、config.yml の manifest_exp_dir が
0032 を指す点だけが違う)。

Downstream: lib.finding_routing に macenko アームを追加する際の索引ソース。
"""
import argparse
import json
import logging
