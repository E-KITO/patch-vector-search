...
experiments/0020(baseline 0018 vs 背景除去前 0002 の A/B、変更なし)と同じ集約
単位・出力構造を踏襲した新規実験。比較軸を「背景除去前後」から「baseline(0018)
vs lib.finding_routing のルーティング結果」に置き換える。

所見(atlas フォルダ)ごとに:
  - GT実測で whiten が優位と確認された所見(README「所見ごとのルーティング表」)
    → routed 列は whiten 索引(異方性除去変換をパッチ単位 exact re-rank にも適用)
  - それ以外(atlas GT 未検証を含む、保守方針) → routed 列は baseline と同一

出力(outputs/0029_.../finding__<slug>/):
  query_tiles/NN_MMM.jpg      クエリ図版のタイル
  hits_baseline/RR.jpg        baseline(0018)索引の第 RR 位パッチ
  hits_routed/RR.jpg          routed(所見によって baseline/whiten)索引の第 RR 位パッチ
  finding.json                所見名・図版数・タイル数・ルーティング先・両索引のヒット表
"""
...
def process_finding(
    *,
    ...
    write_run_metadata(run_dir, exp_name=exp_name, variant_key=variant_key,
                       finding=finding_name, routed_to=routed_to)
    logger.info(f"finding='{finding_name}'  images={len(images)}  routed_to={routed_to}")
    ...
    # --- 各索引で検索 → 実解像度パッチをクロップ ---
    indexes = {"baseline": pi_baseline, "routed": pi_routed}
...
def main() -> None:
    ...
    baseline_index_dir = _staged_or(project_root, config["baseline_index_dir"], "PVS_INDEX_BASELINE_DIR")
    whiten_index_dir = _staged_or(project_root, config["whiten_index_dir"], "PVS_INDEX_WHITEN_DIR")
    ...
    print(f"baseline_index_dir:    {baseline_index_dir}")
    print(f"whiten_index_dir:      {whiten_index_dir}")
    ...
    # atlas フォルダ名 -> コーパス finding_type(CATEGORIES の正規マッピング、GT実測の
    # ある7カテゴリのみ)。マッピングが無いフォルダ(atlas GT未検証)は routed_to=
    # baseline に倒れる(WHITEN_FINDINGS に None が入ることは無いので安全)。
    def _routed_to(finding_name: str) -> str:
        finding_type = CATEGORIES.get(finding_name)
        return "whiten" if finding_type in WHITEN_FINDINGS else "baseline"
    ...
    for finding_name, slug, images in findings:
        routed_to = _routed_to(finding_name)
        pi_routed = pi_baseline if routed_to == "baseline" else pi_whiten
        try:
            r = process_finding(
                finding_name=finding_name, slug=slug, images=images,
                project_root=project_root, exp_name=exp_name, output_root=output_root,
                pi_baseline=pi_baseline, pi_routed=pi_routed, routed_to=routed_to,
                raw_slide_dir=raw_slide_dir, params=params,
        ...
        print(f"[{n_ok + n_skip + n_fail}/{len(findings)}] {slug} (routed_to={routed_to})")

    print(f"sweep done: {n_ok} ok, {n_skip} skipped, {n_fail} failed of {len(findings)}")
    # report.html は run_slurm.sh が続けて scripts/build_atlas_report_routed.py を呼んで生成する。
