...
- [x]  ~~染色正規化(クエリ側・コーパス側)がこの検索タスクに寄与するか~~
      ...
      「可視化の再検証」参照
- [ ]  **uni v2など他モデルでの埋め込みを検討する**(壊死・Kupfferの低迷は
      前処理では動かせない「モデル・コーパスの表現力」の層の問題と判明したため、
      優先度が上がった。下記「可視化の再検証」参照)
...
- [ ]  **抽出できる所見・できない所見を精査する**(壊死・Kupfferは現行の
      `uni_v1`+コーパスでは表現できていないことが分かっているので、まずこの
      「できない所見」の範囲を体系的に確定する)\
など、いろいろやる。
...
- **壊死・Kupfferの低迷は『コーパス/モデルにその所見が無い・distinctiveでない』**という、
  前処理では動かせない層の問題。次の productive な動きは「今後やること」の
  「抽出できる所見・できない所見を精査する」「uni v2など他モデル」。
