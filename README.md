...
- [ ]  **アトラス→TG-GATEsのドメインギャップを詰める**(自己検索は良好なのにアトラスGT比較
      では悪い所見が主因はここ。アトラス図版からの組織タイル抽出の改善、複数図版の
      マルチクエリ化など)
- [ ]  **「機能する ~10 所見」でデータベース化のワークフローを一度通す**(検索精度の
      追い込みと並行して、実際に代表パッチ→検索→パッチ集の構築を end-to-end で試す)\
など、いろいろやる。
