...
## experiments/0037: OvRタイル事前重み付けをコーパスGTの無い所見も含む全25所見に展開(2026-09-12、job 10621)

experiments/0035・0036はコーパスGT正例スライド(`single_finding_liver.csv`)のパッチを
分類器の正例にしていたため、NNLアトラス25所見中GTが確定している7所見にしか適用
できなかった。残り18所見にも同じ枠組みを使えるようにするため、正例の出所を
「コーパスGTスライドのパッチ」から**「atlas図版自体のタイル」**に変更した
(`lib.ovr_scoring.train_and_evaluate_classifier_from_atlas`)。検証もstudy単位の
LOSOから、atlas図版単位のleave-one-image-out(LOIO、「同じ所見の他の図版から
学習して未見の図版を当てられるか」)に変更。25所見全てジョブは正常完了
(`outputs/0037_.../default/summary.csv`)。

- **LOIO AUROCは25所見すべてでほぼ1.0に張り付いた**(fold数2〜10の所見はmin
  0.9977以上、多くは1.0ちょうど)。**Hypertrophyも例外ではなく7fold全てmin=max=1.0**
  ——0036のLOSO(median 0.660、0.10〜0.92でfold間に大きくブレる)とは正反対の結果。
  **これはLOIOという指標そのものの限界を示している可能性が高い**: 正例(atlas図版の
  タイル)と負例(コーパスからのランダムパッチ)は、所見の有無以前に「アトラス由来の
  画像 vs TG-GATEs生WSI由来の画像」という**ドメインそのものが違う**(スキャナ・
  染色・解像度・図版特有のノイズ等)。そのため所見を学習していなくても
  ドメイン差だけで分類器は正例/負例をほぼ完全に分離でき、AUROCが所見の難易度を
  反映しない飽和した指標になっている。この懸念は0037自身のdocstring
  (`lib/ovr_scoring.py`の`loio_auroc_by_image`)でも「コーパス上の実際の正解に
  紐づいた検証ではない」と明記されていたが、今回の全所見一律1.0という結果で
  裏付けが取れた形。**LOIOは所見ごとの分類器品質の切り分けには使えない。**
- **ギャラリー数(unfiltered→ovr_filtered)はコーパスGT対応7所見中6所見で0036と
  完全一致**(Hematopoiesis 0→0、Necrosis 0→1、Glycogen 1→2、Hypertrophy 0→0、
  Increased mitosis 2→3、Kupffer 2→3)——正例の作り方(コーパスGTパッチ→atlas
  タイル)を変えても下流の検索結果は変わらなかった。**Hypertrophyは3つ目の
  独立手法(macenkoルーティング0034・コーパスGT版OvR0035/0036・atlas版OvR0037)
  でもギャラリー0枚のまま**で、所見自体がパッチ単位で識別不能という結論をさらに
  補強する。
  - 唯一の例外は **Inclusion body, intracytoplasmic**(0036: 0→0枚 → 0037: 0→2枚)。
    atlas図版のタイルを正例にしたことで、コーパスGTパッチを正例にした場合より
    多様な典型像を学習できた可能性があるが、n=2枚のギャラリーでは目視レビュー以上の
    判断はできない。
- **残り18所見(コーパスGTなし)はギャラリー数が増減した所見が複数ある**
  (例: Focus 0→1、Intrahepatocellular Erythrocytes 1→2、Hyperplasia,Nodular 0→1)
  が、README「評価に使ったデータとその限界」で述べた19カテゴリの制約
  (コーパスGTが無く検証不能)がそのまま当てはまるため、**LOIOが飽和している今回は
  検索精度が実際に改善したのか判断する手段が無い**(目視で図版とヒットパッチを
  見比べるほかない)。**Fatty Change**(atlas図版8枚、`Degeneration, fatty`の
  コーパス内スライドが0枚)はunfiltered/ovr_filteredとも0→0枚のままで、既存の
  「fatty changeは現行コーパスでは実行不可能」という結論と整合する。
- **結論**: atlas図版を正例にする手法拡張自体は動くが、LOIO検証は所見の学習
  可否を判定する指標として機能しない(飽和)ため、0036のLOSOのような「所見の
  明暗を分ける」用途には使えない。新規18所見のギャラリー変化の妥当性は病理レビュー
  待ちであり、この実験単独では「機能する所見が増えた」とは言えない。次にこの路線を
...
## experiments/0042: Macenko+線形補正併用をアドホック参照タイルで目視確認(2026-09-14、job 10677)

experiments/0041はNNLアトラス図版(所見ラベル付き)を対象にした検証だったが、
atlasのCATEGORIESに属さない単発の参照画像(`data/query/query_001〜003.png`、
experiments/0003以来使われているアドホックなクエリタイル、所見ラベル無し)
でも同じ傾向が出るかを確認した。baseline(plain)/macenko単独/macenko+線形
補正併用(domain_shiftはexperiments/0041のものを再利用、alpha=0.25)の3腕を
実行し、生成されたギャラリー画像を実際に目視した。

- **3クエリとも悪化なし**。3クエリ×3腕全てでギャラリーが生成された(baseline/
  macenko/macenko_domain_correctedいずれも3枚ずつ)。
- **query_003で明確な視覚的改善**: クエリ画像は血管に沿った暗色細胞の線状帯
  (炎症細胞浸潤帯)が特徴的。baselineの上位ヒット(スライド23515、sim
  0.77台)はその構造が弱いパッチが多いのに対し、macenko+線形補正併用の上位
  ヒット(スライド41354、sim 0.82〜0.85)は同様の密な線状炎症細胞帯が複数
  パッチにわたってはっきり写っており、類似度スコアも明確に高い。
- **query_001は軽微な改善**(baselineは特徴的な一致に乏しい一般的な肝細胞
  組織、macenko+線形補正併用は血管周囲の炎症性細胞集塊が見えるパッチを含む)。
- **query_002は両方式が同じ上位スライド(25763)に収束**(sim 0.859→0.866)
  ——特徴的な一致自体は強くないが、手法間の一貫性は高い。
- **注意**: これらはNNLアトラスのようなGT検証済み所見ラベルを持たないアドホック
  画像であり、「正解かどうか」を定量的に断定はできない。あくまで「視覚的に
  もっともらしいか」の確認に留まる。それでも3件中0件が悪化・1件が明確改善という
  結果は、experiments/0041の所見依存的な混在結果と比べてやや前向きな傾向
  ——ただし所見ラベルがある場合の判断(GT+目視)を上書きするものではない。

## experiments/0043: Macenko基準パッチの選定を見直す(2026-09-14、job 10685、CPU-only)

現行の染色正規化基準パッチ(`data/baseline/63958_x38976_y7616.png`、
`lib.finding_routing.MACENKO_STAIN_REFERENCE`)は `scripts/select_average_patch.py`
が2026-08-19頃に一度だけ選んだもので、選定基準は「コーパス40スライド(全体の
4%)からサンプルしたUNI**埋め込み**の重心に最も近いパッチ」だった。これは
Macenko正規化が本来見るべき「染色ベクトル(H&E濃度プロファイル)の典型性」を
直接測ったものではなく(組織の見た目全体を混ぜた埋め込み空間のプロキシ、
スクリプト自身のdocstringも認めている)、選定に使ったパス(`data/raw_wsi`)は
現存せず候補リスト(candidates.json)も残っておらず、背景パッチ除去
(experiments/0017/0018)より前の選定だった。

この実験は、染色ベクトルそのものを直接見る、より原理に忠実な基準で選定を
やり直す診断ツール: 背景除去済みコーパス(experiments/0018)から150スライド
×5パッチ=750枚を生WSIから実際に切り出し、torchstainの
NumpyMacenkoNormalizerで各パッチ自身の染色ベクトル(HERef)・最大濃度
(maxCRef)を推定、その中央値(=コーパス全体の「典型的な染色」)への距離で
ランキングした。現行の基準パッチ自身もこの分布の中での位置を評価した(GPU
不要、UNIエンコーダは呼ばない診断のみ)。

- **結果: 現行の基準パッチは典型性の面でもおおむね妥当だった**。染色ベクトル
  空間での中央値への距離で見て、現行パッチは750パッチ中**上位7.9パーセンタイル**
  (=92%のパッチより中央値に近い)に位置しており、外れ値ではなかった。選定方法
  自体は弱いプロキシだったが、結果的に選ばれたパッチは染色の典型性という観点
  でも十分良い部類だったことになる。
- **新たに見つかった候補**(距離0.067〜0.093、現行の0.191よりさらに中央値に
  近い)を数枚目視したが、現行の基準パッチと同種の、コントラストの良い標準的な
  H&E肝組織で、明確に優れているようには見えなかった——**大きな改善の余地は
  無さそう**。
- **結論**: 染色正規化の不安定さ(README「クエリ側染色正規化の再検証」等)の
  原因は、基準パッチの選定不備ではなかった。「正規化が診断的に意味のある色
  情報(色素沈着・出血など)まで消してしまう」という従来の説明の方が引き続き
  有力。**基準パッチの差し替えは行わない**(そもそも差し替えるには
  `data/trident_processed_uni_v1_macenko`コーパス全体をwsi_preprocess
  パイプラインで再正規化する必要があり、今回はそこまでの動機が無いと判断)。

## experiments/0044: Macenko染色ベクトル推定「パッチ単位 vs スライド単位」の事前検証(2026-09-14、job 10686、CPU-only)

ユーザー提案: 現行は224pxパッチ1枚ごとに独立して染色ベクトル(HERef)・最大
濃度(maxCRef)を推定しているが、スライド(または図版)全体を1回でMacenko
正規化してからタイル分割する方が安定するのではないか。全コーパス再構築
(wsi_preprocessパイプライン、このセッションの実行範囲外)に踏み切る前に、
小規模にこの仮説を検証した。

**設計上の注意(ユーザー指摘)**: 当初「スライド全体」の代理として
`data/trident_processed/thumbnails/`のJPGサムネイルを使う案を検討したが、
JPEG圧縮・ダウンサンプリング時の画素混合が、現行パイプライン(.svsから直接
パッチを切り出す)とは異なる色処理経路になり、「パッチ単位 vs スライド単位」
という本来の比較軸に「.svs直接 vs JPG経由」という別の交絡が混入するため却下
した。代わりに、同じ.svsから`lib.raw_patch.crop_patch`で直接切り出した
パッチのみを使い、(a)パッチごとに個別fit(現行方式)と、(b)同じスライドの
パッチ20枚を1枚に連結してから1回だけfit(スライド単位方式)を比較した——
色処理経路を完全に同一にした上で、fitに使うピクセル量(粒度)だけを変えた。
30スライド×20パッチ=600パッチ、GPU不要・wsi_preprocessも不要
(`torchstain`+`lib.raw_patch`のみで完結)。

- **結果**: パッチ内不安定さ(同一スライド内でのパッチ間ばらつき、mean 0.289・
  median 0.222)は、比較の物差しとして測った**スライド間の本物の染色差**
  (mean 0.897・median 0.795)の**約28〜32%**だった。ノイズは実在するが、
  保存すべき本物の信号を覆い隠すほどではない——「圧倒的な問題」でも
  「無視できるノイズ」でもない、中間的な結果。
- 30スライド中29スライドでは「パッチ個別fitの平均」と「スライド単位(連結)
  fit」がほぼ一致し、スライド単位fitが妥当な要約になっていることを裏付けた。
  1スライド(48062)のみ外れ値(パッチ内ばらつき0.445に対しスライド単位fit
  への乖離1.241)で要注意。
- **結論**: スライド単位fitへの切り替えは正規化ノイズを一定程度減らす可能性
  がある中程度の根拠が見つかった。ただし全コーパス再構築に見合う効果かは
  この規模の事前検証だけでは判断できず、実際のGT best_rank改善を見るには
  wsi_preprocessパイプラインでの本番再構築が必要——**ユーザーの意向により
  実際にコーパス再構築を進める方針**(下記「wsi_preprocess連携」参照)。

## wsi_preprocess連携: コーパス再構築の実行範囲について

`wsi_preprocess`(コーパスのraw WSI→パッチ抽出→染色正規化→UNI埋め込みを担う
外部パイプライン)は、このリポジトリ内には存在しない別リポジトリ/環境で
運用されている(このcapsule環境から`find`しても見つからない)。全1000スライド
・約1800万パッチの再クロップ→染色ベクトル推定→再埋め込みという規模の処理は、
このClaude Codeセッション(Slurm実験としての軽量ジョブ投入が前提のcapsule
環境)の実行範囲外であり、`wsi_preprocess`を操作できるチーム/環境側での実行が
