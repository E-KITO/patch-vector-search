...
- **どの所見で機能するか**: glycogen は機能する(ランダム対照で判定精度91%)。
  **hypertrophy は不成立**(同59%、chance 以下)。所見は「パッチ内で完結するテクスチャ」
  「相対的基準を要する」「sub-patch 局所」の3クラスに分かれ、機能するのは最初の1つだけ。
  下記「所見の3クラス分け」参照。**GT recall だけでは判定を誤るので、0015 の後は必ず
...
| | glycogen (job 9943) | hypertrophy (job 9937) |
|---|---|---|
| キュレート集合 | deliver 137枚 / 30スライド | validate 150枚 / 49スライド |
| 100枚中の内訳 | curated 50 / random 50 | curated 61 / random 39 |
| **判定精度** | **91.0%** | **59.0%** |
| ベースライン(多数派を選ぶ) | 50.0% | 61.0% |
| curated の recall | 98% (49/50) | 52% (32/61) |
| curated の precision | 86% (49/57) | 73% (32/44) |
...
|---|---|---|
| whole-patch テクスチャ(パッチ内で完結) | glycogen(検証済み), fatty change?, ground glass? | **機能する**(seed が動く側なら) |
| 相対的基準を要する | **hypertrophy**, 萎縮 | **機能しない**(下記) |
...
2. **fatty change, ground glass で 0015 を回す** — ただし **0015 の後に必ず
   `scripts/random_patch_baseline.py` でランダム対照を取ること**。GT 指標(recall・
   chance 比)だけでは hypertrophy のケースを弾けない
3. **deliver モードの候補枯れ対策** — glycogen deliver は候補 174 で target 150 に届かず
   137枚止まりだった。他の所見に展開するなら `k_candidate_patches` を上げる必要がある
   (上記「候補プールは固定窓」参照)
4. hypertrophy を扱うなら、パッチサイズを上げて基準組織を同一視野に入れる等、
