...
- **成果物パイプライン**: `lib/patch_set.py` + `experiments/0015` で、所見の確定スライドを
  ...
  未ラベルスライドのみから137枚を構成済み(病理レビュー待ち)。ただし機能するのは
  whole-patch テクスチャ系の所見に限られる。下記「成果物の試作」参照。
...
- [~]  **「機能する所見」でデータベース化のワークフローを一度通す**
      ...
      所見の性質上不成立(所見の2クラス分けが見えた)。2026-09-05、0015 に validate/deliver
      モードを分離し、**glycogen で deliver(未ラベルスライドのみから137枚)まで到達**。
      hypertrophy への展開は判定中。継続中。詳細は下記「成果物の試作」参照
